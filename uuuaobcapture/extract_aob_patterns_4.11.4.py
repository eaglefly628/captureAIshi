...
GAME_TAGS = {
    # process-name strcmp targets in UE4 unlocker 4.11.5
    ...
    # short-form strcmp / startswith targets
...
ADDAOB_WRAPPERS = ["1800ea000", "1800ea1c0", "1800ea270", "1800ea330"]
...
for aob_name_list in list(game_data["Shared / UE framework (all games)"].keys()):
    for prefix, game in PREFIX_TO_GAME:
        if aob_name_list.startswith(prefix):
            game_data[game][aob_name_list] = game_data["Shared / UE framework (all games)"].pop(aob_name_list)
            break
...
OUT = r"D:\dev\uuuaobcapture\aob_patterns_by_game4.11.4.json"
