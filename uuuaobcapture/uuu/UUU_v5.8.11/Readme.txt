...
==============
v5.8.11:
    - Added: (Experimental for now) A bounding box is now displayed around the selected actor (off by default). This is the first step in adding multi-selection 
             for actors. 
    - Changed: Adjusted the width of controls on the actor manipulation tab a bit so there's less empty space on the right side 
    - Fixed: Updated for Stalker 2 patch released on April 14th 
    - Added: Gamepad input blocking for Stalker 2 because the people at GSC implemented a GameInput wrapper and don't use the Unreal Engine input pipeline. 

v5.8.10:
    - Added: Added support for frankenstein engine variants with UE4 internals and UE5 rendering pipelines. 
    - Added: Added a 'Select Player Pawn' button to the actor manipulation tab for the situations where selecting the player isn't working for some reason 
             (e.g. the engine reports no actor under the cursor) 
    - Added: Added a workaround for an Unreal Engine issue where switching a mesh on a skeletal mesh component would re-use the original materials. This 
             workaround makes the skeletal mesh component use the materials of the new mesh so if you e.g. swap skins on an actor, and the new skin has 
             different materials, they're now used properly 

v5.8.9:
    - Fixed: Made a customization for Titan Quest II's early access build 
    - Fixed: After loading a pose and then applying meta-human expressions, it could be parts of the face would skew or go into the wrong position. 
    - Fixed: Changed the version detection code to be more reliable so it should result in less crashes with 'Fatal Error' after injecting 

v5.8.8:
    - Fixed: Fixed an issue where changing directional light visibility and color directly on the Atmospherics tab wouldn't reset to the in-game value 
             when editing was disabled 
    - Fixed: If a game had multiple exponential height fog components, sky atmosphere components or volumetric cloud components defined in the world but 
             just one visible, the UUU would previously pick the first one it found. It will now pick the one which is actually visible in the world so 
             the editor will actually do something in these cases. 
    - Fixed: It could be the changes made to 5.8.7 to support UE 5.7+ would cause a crash in specific UE 5.4/5.5 games. 
    - Fixed: Added more safeguards for the objectsstore item size, which might make the UUU work in internal test builds of a game. 
    - Fixed: Made some changes to actor spawning to make it more reliable (e.g. when creating lights) to make sure the call originates from the game thread. 
             While this isn't strictly needed in newer UE5 versions, it could be the game would crash it would check deep in the call stack if it was called 
             on the gamethread or not. 
    - Fixed: Added a lot of workarounds for Avowed but some parts aren't working anymore (pose tools aren't working anymore as well as the hud toggle. 
             The functions for the pose tools are in-lined and that's currently not supported in UUU v5) 
