/*
 * captureAIshi_bridge.addon  --  Path B (ReShade addon vehicle)
 *
 * Independent copy of 3rdparty/bridge/src/bridge.cpp. Same TCP 9998 protocol,
 * same GEngine auto-scan, same camera/HUD/path control. Only the injection
 * vehicle differs:
 *   Path A: renderdoccmd inject captureAIshi_bridge.dll
 *   Path B: dxgi.dll proxy + this addon (this file)
 *
 * Driver and UI are oblivious -- they speak the same protocol.
 *
 * Why a copy and not shared headers? Per project decision: keep the two
 * vehicles fully independent so changes to A cannot regress B and vice versa.
 * Bug fixes apply to both files manually.
 *
 * Phase 0 scope: bridge logic only. ReShade frame-capture events are NOT
 * wired here yet -- arrives in Phase 1. The addon registers with ReShade so
 * it loads under dxgi.dll proxy, then runs the exact same TCP server /
 * engine-scan / camera-control pipeline as Path A.
 *
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID reserved)
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        /* Register with ReShade so it loads us as an addon when dxgi.dll
