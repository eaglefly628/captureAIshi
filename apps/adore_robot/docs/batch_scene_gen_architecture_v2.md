...
| LLM provider | Hard-coded Anthropic Claude Sonnet | **`apps/adore_robot/llm/factory.py` -- `make_llm_client(provider)` returns a `BaseLLMClient` with `chat_with_tools(...)`.** Default `deepseek` (V4-Flash). Switchable to `anthropic`, `qwen`, `qwen-vl`, `glm`, `kimi`, `doubao`. | DeepSeek V4-Flash ~10x cheaper than Claude Sonnet 4.6, stable tool calling. NL->delta task is light; premium reserved for hard reasoning. Multimodal (thumbnail feedback v0.4) defaults to `qwen-vl`. |
