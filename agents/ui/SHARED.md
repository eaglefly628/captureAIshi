...
- [x] **P1: a5f8859 export_batch stderr 管道风险** (spotted by 小萱, fixed by 主程序员) — 改为 `stderr=subprocess.STDOUT`，合并到 stdout readline 循环。

- [x] **P0 安全: game config slug 路径穿越** (spotted by 主程序员) — Fixed: `get_game_config` 和 `save_game_config` 未对 URL 参数 `slug` 调用 `_game_slug()` 清洗，攻击者可用 `../` 读写任意 JSON。已加 `_game_slug(slug)` + 空值检查。
- [x] **P1: path_player add_node 索引越界** (spotted by 主程序员) — Fixed: `index >= len(cp.nodes)` 改为 `index > len(cp.nodes)`，允许 `insert(len, node)` 在末尾插入。
- [ ] **P2: path_player quaternion clamp 不完整** (spotted by 主程序员) — SLERP 中 `dot = min(dot, 1.0)` 只夹上界，浮点误差可能导致 `dot < -1.0`。应改为 `np.clip(dot, -1.0, 1.0)`。
- [ ] **P2: web_ui profile name 缺验证** (spotted by 主程序员) — `/api/config/profiles/<name>` 的 `name` 参数未做路径安全校验，可能被 `../` 穿越。建议加 `re.match(r'^[a-zA-Z0-9_-]+$', name)` 校验。
