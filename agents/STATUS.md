...
| 小幻 (PCG) | ~25% | 2026-05-19 | v0.3.3 接力 done (commit 5b62b77): (1) PG_Warehouse_v0 节点级 design doc 11 参数 -> 8 Stage 全接入 (2) contract §9 AICallable pointer + 边界规则防双源漂移 (3) §1.0 Common to ALL scenes 收编 d45a3af3 drift (4) §4.2 Example 6 MCP tool call 序列. **新 P0 (xiaoxu via 用户 2026-05-17)**: 真 PG_Warehouse/_LivingRoom/_IndustrialCorner .uasset + sample .umap. **Sandbox 无 UE Editor 不可直出 .uasset**, 待回 xiaoxu/老白提**替代路径**: 我把 design doc 改写成 MCP `execute_tool_script` 可消费的 Python build script, 让 xiaoxu MCP 端跑脚本 in-Editor 自动建 asset, 否则等 xiaoxu 手搭. |
