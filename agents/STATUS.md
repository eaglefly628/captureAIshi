...
| 小幻 (PCG) | ~67% | 2026-05-15 | v0.3.2 P0 Step 1-4 + P1 from xiaoxu 6 项全 done (e484db1)；补丁: 老白 5f1cfa2c P0 反向影响 -- 三场景 JSON 加 `robotics_backend` + `robotics_backend_compatible` 字段，contract 加 §7 (5 节) + 重编号旧 §7→§8，写 P1 到 xiaoxu SHARED 要 batch UI 多 backend 显式表达 (7 条: selector / status badge / /api/scenes / 默认 minimal / NL 不暴露 / scene JSON 已就位 / 表单位置)。单点依赖仍是 xiaoxu 装 5.8 后回填 Python method 真名。|
