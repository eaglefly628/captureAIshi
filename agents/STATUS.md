...
| 小幻 (PCG) | ~38% | 2026-05-19 | v0.3.3 用户路线: (a39e0f8) `demo_v0_simplified_contract.md` v0 demo 砍到 3 actor 动作 (spawn/delete/modify_location) + 1 query (list_objects) + 6 mesh 资产目录 + LLM system prompt + 6 few-shot + UE map 准备 + 5 步演示验收. (本轮 §10 增补): 用户 "把整个场景物件布局生成呢" -- 3 个 layout 生成器 (warehouse 完整算法 ~95 行 Python / living_room+industrial_corner sketch) + clear_demo_objects + 3 个 few-shot. warehouse 默认参数 37 actor 一次 generate ~4s, generate 后继续用 §2 primitive 微调. xiaoxu 接力 wire 4 个新 tool + prompts.py 补丁. v1 PG_Warehouse 11-param 路线档存等演示走通后接. |
